...
db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS menfess_sessions (
        ...
        user1_jid TEXT NOT NULL UNIQUE,
        user2_jid TEXT NOT NULL UNIQUE,
...
export const menfessSessionCache = new LRUCache({ max: 200, ttl: 1000 * 60 * 5 });
...
const _statements = {
    ...
    getMenfessSession: db.prepare('SELECT * FROM menfess_sessions WHERE user1_jid = ? OR user2_jid = ?'),
    startMenfessSession: db.prepare('INSERT INTO menfess_sessions (user1_jid, user2_jid, expires_at) VALUES (?, ?, ?)'),
    endMenfessSession: db.prepare('DELETE FROM menfess_sessions WHERE user1_jid = ? OR user2_jid = ?'),
    cleanupExpiredMenfess: db.prepare('DELETE FROM menfess_sessions WHERE expires_at < ?'),
